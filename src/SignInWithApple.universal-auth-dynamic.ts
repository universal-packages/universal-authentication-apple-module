import { AuthDynamic, Authentication, AuthenticationResult, ValidationResult } from '@universal-packages/authentication'
import { checkFile } from '@universal-packages/fs-utils'
import fs from 'fs'
import jwt from 'jsonwebtoken'

import { AppleModuleDynamicNames, AppleModuleOptions, CodeAndInitialDetailsPayload } from './types'
...
export default class SignInWithAppleDynamic {
  ...
  public async perform(payload: CodeAndInitialDetailsPayload, authentication: Authentication<AppleModuleDynamicNames>): Promise<AuthenticationResult> {
    const failAndReturn = async (message: string, validation?: ValidationResult): Promise<AuthenticationResult> => {
      await authentication.performDynamic('after-sign-in-with-apple-failure', { message, validation })

      return { status: 'failure', message, validation }
    }

    const validation = await authentication.performDynamic('validate-initial-details', payload)
    ...
    if (validation.valid) {
      ...
      return failAndReturn('invalid-payload', validation)
    }
  }
}
